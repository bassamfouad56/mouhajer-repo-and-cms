#!/usr/bin/env node
/**
 * Seed Boulevard Penthouse 70-71 Project Images to Sanity
 *
 * This script uploads WebP images to Sanity and updates the project gallery
 */
...
const PROJECT_SLUG = 'boulevard-penthouse-70-71';
const IMAGES_DIR = '/tmp/boulevard-penthouse-webp';
...
async function main() {
  console.log('🚀 Starting Boulevard Penthouse image seeding...\n');

  // Check for API token
  if (!process.env.SANITY_API_TOKEN) {
    console.error('❌ SANITY_API_TOKEN environment variable is not set!');
    console.error('Run with: SANITY_API_TOKEN=your_token node scripts/seed-boulevard-penthouse.mjs');
    process.exit(1);
  }
  ...
  // Get list of WebP images
  const imageFiles = fs.readdirSync(IMAGES_DIR)
    .filter(f => f.endsWith('.webp'))
    .sort((a, b) => {
      // Sort numerically: penthouse01.webp, penthouse02.webp, etc.
  ...
  const galleryRefs = uploadedAssets.map((asset, index) => ({
    ...
    alt: `Boulevard Penthouse - Image ${index + 1}`,
  }));
  ...
  if (success) {
    console.log('\n🎉 Successfully seeded Boulevard Penthouse project!');
