#!/usr/bin/env node
/**
 * Seed Palm Jumeirah Villa Project Images to Sanity
 */
...
const PROJECT_SLUG = 'private-villa-palm-jumeirah';
const IMAGES_DIR = '/tmp/palm-jumeirah-webp';
...
async function main() {
  console.log('🚀 Starting Palm Jumeirah Villa image seeding...\n');
  ...
  const galleryRefs = uploadedAssets.map((asset, index) => ({
    ...
    alt: `Palm Jumeirah Villa - Image ${index + 1}`,
  }));
  ...
  if (success) {
    console.log('\n🎉 Successfully seeded Palm Jumeirah Villa project!');
