...
async function assignImagesToContent() {
  ...
  try {
    ...
    console.log(
      `   Found ${awardsWithoutImages.length} awards without images\n`
    );

    // Step 3: Assign random images to awards
    let assignedToAwards = 0;
    for (const award of awardsWithoutImages.slice(
      0,
      Math.min(awardsWithoutImages.length, allImages.length)
    )) {
    ...
    for (const project of projectsWithoutImages) {
      ...
      if (projectImages.length > 0) {
        ...
        console.log(
          `   ✅ Assigned ${projectImages.length} images to project: ${project.title}`
        );
        assignedToProjects++;
      }
    }
