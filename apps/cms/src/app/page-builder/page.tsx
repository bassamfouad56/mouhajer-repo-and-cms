/**
 * Advanced Page Builder with SEO Optimization
...
export default function PageBuilderPage() {
  ...
  // Auto-translate English title to Arabic
  useEffect(() => {
    if (!newPage.titleEn) {
    ...
    const timeoutId = setTimeout(async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setNewPage(prev => ({
            ...prev,
            titleAr: data.translatedText,
            slugAr: data.translatedText.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, ''),
          }));
        }
      } catch (error) {
                    ...
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <div className="flex items-center space-x-1">
                        <div className="w-16 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full ${
                              (page.seoMetaTitleEn && page.seoMetaDescEn)
                                ? 'bg-green-500 w-4/5'
                                : 'bg-yellow-500 w-2/5'
                            }`}
                          />
                        ...
                        <span className="text-gray-700 font-medium">
                          {(page.seoMetaTitleEn && page.seoMetaDescEn) ? '80%' : '40%'}
                        </span>
                      </div>
                    </div>
                  </div>
            ...
            <div className="max-w-5xl mx-auto p-8">
              ...
              <div className="bg-white rounded-2xl shadow-sm p-6 mb-6 border border-gray-200">
                <div className="flex items-start justify-between">
                  ...
                  <button className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors font-medium shadow-sm">
                    Publish
                  </button>
                </div>
              </div>
                        ...
                        onSave={async (instances) => {
                          try {
                            const response = await fetch(`/api/pages/${selectedPage.id}/components`, {
                              method: 'PUT',
                              headers: { 'Content-Type': 'application/json' },
                              body: JSON.stringify({ components: instances }),
                            });
              ...
              <div className="space-y-4 pt-6 border-t border-gray-200">
                ...
                <div>
                  ...
                  <input
                    ...
                    readOnly
                    placeholder="auto-generated-from-arabic-title"
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-lg bg-gray-50 text-gray-600 cursor-not-allowed font-mono"
                  />
                  <p className="mt-1 text-xs text-gray-500">
                    Auto-generated from Arabic title (or uses English slug if Arabic title is empty)
                  </p>
                </div>
